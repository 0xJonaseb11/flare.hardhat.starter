...
contract SwapAndRedeem {
    ...
    )
  {
    ...
    (uint256 _amountIn, uint256 _amountOut) = calculateRedemptionAmountIn(_lots);

    require(token.balanceOf(msg.sender) >= _amountIn, "Insufficient token balance to swap");

    // Check if the user has enough FXRP allowance
    require(token.allowance(msg.sender, address(this)) >= _amountIn, "Insufficient FXRP allowance");

    // Transfer tokens from msg.sender to this contract to swap to FXRP
    require(token.transferFrom(msg.sender, address(this), _amountIn), "Transfer failed");
  ...
  function calculateRedemptionAmountIn(uint256 _lots) public view returns (uint256 amountOut, uint256 amountIn) {
    ...
    uint256[] memory amounts = router.getAmountsIn(lotSizeAMG * _lots, swapPath);
  ...
  function _swap(
    ...
    address[] memory path,
    uint256 deadline
  ) internal returns (uint256[] memory amountsSent, uint256[] memory amountsRecv) {
